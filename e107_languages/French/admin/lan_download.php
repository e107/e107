<?php
/**
 * Fichiers utf-8 français pour le CMS e107 version 0.8 α
 * accessoirement compatible 0.7.11
 * Licence GNU/GPL
 * Traducteurs: communauté française e107 http://etalkers.tuxfamily.org/
 *
 * $Source: /cvsroot/touchatou/e107_french/e107_languages/French/admin/lan_download.php,v $
 * $Revision: 1.15 $
 * $Date: 2009/06/05 13:24:40 $
 * $Author: marj_nl_fr $
 */
if (!defined('e107_INIT')) { exit(); }

define('DOWLAN_1', 'Téléchargement ajouté dans la base de données.');
define('DOWLAN_2', 'Téléchargement mis à jour dans la base de données.');
define('DOWLAN_3', 'Téléchargement supprimé.');
define('DOWLAN_4', 'Veuillez cocher la case pour confirmer la suppression du téléchargement');
define('DOWLAN_5', 'Il n’y a pas encore de catégorie de téléchargement définie. Tant qu’aucune catégorie ne sera créée vous ne pourrez pas ajouter de téléchargements.');
define('DOWLAN_6', 'Aucun téléchargement pour le moment');
define('DOWLAN_7', 'Téléchargements existants');

define('DOWLAN_11', 'Catégorie');
define('DOWLAN_12', 'Nom');
define('DOWLAN_13', 'Fichier');
define('DOWLAN_14', 'entrer l’URL si le fichier est externe');
define('DOWLAN_15', 'Auteur');
define('DOWLAN_16', 'Email');
define('DOWLAN_17', 'Site internet');
define('DOWLAN_18', 'Description');
define('DOWLAN_19', 'Image principale');
define('DOWLAN_20', 'Image réduite');
define('DOWLAN_21', 'Statut');

define('DOWLAN_24', 'Mettre à jour le téléchargement');
define('DOWLAN_25', 'Proposer le téléchargement');

define('DOWLAN_27', 'Téléchargement');
define('DOWLAN_28', 'Aucun');
define('DOWLAN_29', 'Page principale des téléchargements');
define('DOWLAN_30', 'Créer un téléchargement');
define('DOWLAN_31', 'Catégories');
define('DOWLAN_32', 'Options Téléchargements');
define('DOWLAN_33', 'êtes-vous sûr de vouloir supprimer ce téléchargement?');
define('DOWLAN_34', 'êtes-vous sûr de vouloir supprimer cette catégorie de téléchargement?');

define('DOWLAN_36', 'Supprimé');
define('DOWLAN_37', 'Catégorie');
define('DOWLAN_38', 'Aucune catégorie n’existe');
define('DOWLAN_39', 'Catégories de téléchargement');
define('DOWLAN_40', 'Aucune catégorie principale');
define('DOWLAN_41', 'Icône');
define('DOWLAN_42', 'Voir les icônes');
define('DOWLAN_43', 'Visible par');
define('DOWLAN_44', 'La catégorie n’est visible que par les membres du groupe que vous aurez sélectionné');
define('DOWLAN_45', 'Créer la catégorie');
define('DOWLAN_46', 'Mettre à jour la catégorie');
define('DOWLAN_47', 'Catégorie créée');
define('DOWLAN_48', 'Catégorie mise à jour');
define('DOWLAN_49', 'Catégorie des téléchargements');

define('DOWLAN_51', 'Chercher/Rafraichir les téléchargements');
define('DOWLAN_52', 'Fichiers');
define('DOWLAN_53', 'Sous-catégorie');
define('DOWLAN_54', 'Options des Téléchargements');
define('DOWLAN_55', 'Nombre de téléchargements à afficher par page');
define('DOWLAN_56', 'Tri par ');


define('DOWLAN_59', 'Nom du fichier');

define('DOWLAN_62', 'Croissant');
define('DOWLAN_63', 'Décroissant');
define('DOWLAN_64', 'Mettre à jour les options');
define('DOWLAN_65', 'Options mises à jour');
define('DOWLAN_66', 'Taille du fichier');
define('DOWLAN_67', 'ID');
define('DOWLAN_68', 'Fichier inexistant!');
define('DOWLAN_69', 'Téléchargements traités par le PHP');
define('DOWLAN_70', 'Cochez pour envoyer toutes les demandes de téléchargement via PHP.');
define('DOWLAN_100', 'Activer l’accord de téléchargement');
define('DOWLAN_101', 'Texte de l’accord');
define('DOWLAN_102', 'Autoriser les commentaires?');
define('DOWLAN_103', 'Enlever des téléchargements');
define('DOWLAN_104', 'a été enlevé des téléchargements');
define('DOWLAN_105', 'Retour vers les Téléchargements');
define('DOWLAN_106', 'Téléchargement accessible à');
define('DOWLAN_107', 'Limitation du nombre de téléchargement');
define('DOWLAN_108', 'Limitation de la largeur de bande du téléchargement');
define('DOWLAN_109', 'pour');
define('DOWLAN_110', 'jours');
define('DOWLAN_111', 'ko');
define('DOWLAN_112', 'Limitations');
define('DOWLAN_113', 'Groupe d’utilisateurs');
define('DOWLAN_114', 'Ajouter une nouvelle limitation');
define('DOWLAN_115', 'Mettre à jour la limitation');
define('DOWLAN_116', 'Limitation déjà existante pour cette classe d’utilisateurs');
define('DOWLAN_117', 'Limitation mise en place avec succès');
define('DOWLAN_118', 'Limitation non mise en place. Erreur inconnue');
define('DOWLAN_119', 'Limitation désactivée avec succès ');
define('DOWLAN_120', 'Limitation non désactivée. Erreur inconnue');
define('DOWLAN_121', 'Limitation mise à jour avec succès');
define('DOWLAN_122', 'Inactif');
define('DOWLAN_123', 'Actif: Fichier sujet à des limitations de téléchargement');
define('DOWLAN_124', 'Actif: Fichier non sujet à des limitations de téléchargement');
define('DOWLAN_125', 'Limitation de téléchargements activée');
define('DOWLAN_126', 'Statuts d’activation mis à jour');
define('DOWLAN_127', 'Entrez seulement à la taille du fichier si le téléchargement est un fichier externe');
define('DOWLAN_128', 'Miroirs');
define('DOWLAN_129', 'laisser en blanc si vous n’utilisez pas de miroirs');
define('DOWLAN_130', 'Ajouter un autre miroir');
define('DOWLAN_131', 'Sélectionner le fichier local');
define('DOWLAN_132', 'Entrer un miroir à utiliser, puis l’adresse de téléchargement');

define('DOWLAN_133', 'Miroir mis à jour en base de données');
define('DOWLAN_134', 'Miroir sauvegardé en base de données');
define('DOWLAN_135', 'Miroir supprimé');
define('DOWLAN_136', 'image');
define('DOWLAN_137', 'êtes-vous sûr de vouloir supprimer ce miroir?');
define('DOWLAN_138', 'Miroir existants');
define('DOWLAN_139', 'Adresse');
define('DOWLAN_140', 'Télécharger des images locales dans e107_files/downloadimages pour les afficher ici, ou entrez l’adresse complète si l’image à été déplacée');
define('DOWLAN_141', 'Lieu');
define('DOWLAN_142', 'Mettre à jour les Miroirs');
define('DOWLAN_143', 'Créer un Miroir');
define('DOWLAN_144', 'Pas de serveur miroir défini dans la section miroirs.');

define('DOWLAN_145', 'Téléchargement visible par');
define('DOWLAN_146', 'Message d’avertissement personnalisé ou URL pour les téléchargement');

define('DOWLAN_147', 'Icône pour catégorie vide');
define('DOWLAN_148', 'Cocher pour mettre la date à aujourd’hui');
define('DOWLAN_149', 'Ou cliquez ici pour utiliser un fichier externe');
define('DOWLAN_150', 'Prévenir l’admin lorsqu’un téléchargement est annoncé comme défectueux');
define('DOWLAN_151', 'Autoriser le signalement des téléchargements défectueux pour');

define('DOWLAN_152', 'Ne peut déplacer le fichier');
define('DOWLAN_153', 'Déplacer vers le dossier de téléchargement');

define('DOWLAN_154', 'si les miroirs sont utilisés, choisissez comment les afficher');
define('DOWLAN_155', 'Type affichage des miroirs:');
define('DOWLAN_156', 'Afficher la liste des miroirs. Permet à l’utilisateur de choisir.');
define('DOWLAN_157', 'Utiliser un miroir aléatoirement. Pas de choix utilisateur.');

define('DOWLAN_158', 'Afficher les sous-sous-catégories sur la page principale de téléchargement.');
define('DOWLAN_159', 'Inclure le total de sous-sous-catégories dans le compte de sous-catégorie.');

define("DOWLAN_160", 'Empêcher les téléchargements simultanés pour un utilisateur unique');